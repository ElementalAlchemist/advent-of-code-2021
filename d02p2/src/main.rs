...
impl Instruction {
	fn process(&self, position: &mut u32, depth: &mut u32, aim: &mut u32) {
		match self.direction {
			Direction::Forward => {
				*position += self.distance;
				*depth += *aim * self.distance;
			}
			Direction::Down => *aim += self.distance,
			Direction::Up => *aim -= self.distance,
		}
	}
}
...
fn main() {
	...
	for instruction in instructions.iter() {
		instruction.process(&mut &mut horizontal_position, &mut depth, &mut aim);
	}
