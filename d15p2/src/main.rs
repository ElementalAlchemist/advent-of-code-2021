...
fn main() {
	let initial_risk_map: Vec<Vec<u32>> = fs::read_to_string("input.txt")
		.unwrap()
		.lines()
		.filter(|s| !s.is_empty())
	...
	for r in 0..5 {
		for row in initial_risk_map.iter() {
			let mut current_row = Vec::new();
			for c in 0..5 {
				for column in row.iter() {
					let mut new_value = *column + r + c;
					if new_value > 9 {
						new_value -= 9;
					}
					current_row.push(new_value);
				}
			}
			risk_map.push(current_row);
		}
	}
